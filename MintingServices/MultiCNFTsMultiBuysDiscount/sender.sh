...
while true; do
  ...
  if [ $check -gt 0 ]
  then
    cardano-cli query protocol-parameters $networkParam --out-file protocol${session}.json
    ...
    do
      ...
      discountSQL=$(psql -U ubuntu -d ${databaseName} -t -A -c "select discount from sender where hash = '${hash}';")
      if [ $discountSQL -eq 0 ]
      then
        costTotal=$(($nftCost * $buys))
        totalLovelaces=$(expr $totalLovelaces + $costTotal)
      else
        costTotal=$(($discountNftCost * $buys))
        totalLovelaces=$(expr $totalLovelaces + $costTotal)    
      fi
    done
    ...
    do
      ...
      echo session ${session}    address $receivingAddress

      discountSQL=$(psql -U ubuntu -d ${databaseName} -t -A -c "select discount from sender where hash = '${hash}';")
      echo session ${session}    buys $buys
      echo session ${session}    discount $discountSQL
      ...
      do
        ...
        echo session ${session}    RandomCNFT ${nftProject}${random}
        #IPFS
        ...
        then
          ...
          while [ "$abc" = "" ]
          do
            calcMinVal=$(cardano-cli transaction calculate-min-required-utxo --protocol-params-file protocol${session}.json --tx-out "${multiAsset}" $era)
            abc="abc"
            if [ $? -ne 0 ]; then abc=""; fi
          done
    ...
    while [ "$abc" = "" ]
    do
      minfee=$(cardano-cli transaction calculate-min-fee --tx-body-file tx${session}.raw --tx-in-count $txInCounter --tx-out-count $txOutCounter --witness-count 1 --byron-witness-count 0 $networkParam --protocol-params-file protocol${session}.json | awk '{ print $1 }');
      abc="abc"
      if [ $? -ne 0 ]; then abc=""; fi
    done
